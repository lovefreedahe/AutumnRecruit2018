...
* https在TCP协议之上加入了SSL/TLS协议，该协议有如下作用：
    * 内容加密
    * 身份验证：验证网站的真实性
    * 完整性确认：防止传输信息被篡改
* 由于https加入了如上功能，所以会变得比http慢，google研发了SPDY协议，在SSL/TLS之上，有如下功能：
    * 多路复用：一个TCP连接可以同时传输多路数据，提高了利用率，降低延迟
    * header压缩
    * 服务器推送
* 后来出现了http2.0 ，其和SPDY差不多，不过多了二进制传输的功能。
11. IP地址子网划分
<div align="center"><img src="../../resources/images/network/ziwang.png" ></div>

    技巧：一半 一半 再一半
    
    192.168.0.101/27 
<div align="center"><img src="../../resources/images/network/ziwang2.png" ></div>

12. POST和GET区别
* get参数在url中，post在request body中
* get参数有长度限制，post没有
* get直接将参数暴露在url中，所以不安全
...
* 读本机hosts文件
* 请求DNS服务器
    1. ISP(Internet Service Provider)的DNS服务器
    2. root服务器递归查询
## 深入部分 
1. TCP如何保证数据的可靠传输的（这个问题可以引申出很多子问题，拥塞控制慢开始、拥塞避免、快重传、滑动窗口协议、停止等待协议、超时重传机制，最好都能掌握） 
* 确认和重传：没收到一个报文就会确认，发送方发送一段时间没确认就会重传
* 数据校验
* 数据合理分片和排序：TCP会缓存未按序到达的报文，重新排序后交给应用层
* 流量控制：
当接收方来不及处理发送方的数据，能提示发送方降低发送的速率，防止包丢失。
* 拥塞控制：
    * 慢开始和拥塞避免
    开始cwd大小为1，然后指数增长，当超过慢开始门限ssthresh后，逐步加1，当出现超时，便将ssthresh变为cwd/2，重新开始慢开始，cwd为最大报文长度。
    * 快重传和快恢复
    当接受方按顺序确认M1,M2,M3,M4时，接收到M2然后是M4，接收方没有收到M3，会连续发送M2的确认，发送方会执行快重传，同时快恢复将ssthresh=cwd/2,cwd=ssthresh。
2. 地址解析协议ARP 
